...
bool NetworkMgr::init(ServerConfig* serverConfig) {
  ...
  if (!serverConfig->getConfig(serverRoot)) {
    ...
    // Pertimbangkan penanganan error, misalnya kembali ke default atau return
    // false
  }

  // 2. Tentukan primaryMode (Logika ini bisa tetap, atau disesuaikan jika
  // 'mode' juga pindah) Periksa apakah field 'communication' masih ada dan
  // berisi 'primary_network_mode' atau 'mode'
  String oldMode = "";
  ...
  bool ethernetEnabled =
      ethernetConfigPresent && (ethernetConfig["enabled"] | false);

  // Always initialize WiFi library (needed for WiFiClient even when using
  // Ethernet)
  WiFi.mode(WIFI_STA);
  ...
  if (primaryMode == "ETH" && ethernetManager &&
      ethernetManager->isAvailable()) {
    activeMode = "ETH";
  } else if (primaryMode == "WIFI" && wifiManager &&
             wifiManager->isAvailable()) {
    activeMode = "WIFI";
  } else if (ethernetManager &&
             ethernetManager->isAvailable()) {  // Fallback ke Ethernet jika ada
    activeMode = "ETH";
  } else if (wifiManager &&
             wifiManager->isAvailable()) {  // Fallback ke WiFi jika ada
    activeMode = "WIFI";
  } else {
    activeMode = "NONE";
  }
  ...
  return true;  // Kembalikan true jika inisialisasi (walaupun tanpa koneksi)
                // berhasil
}
...
bool NetworkMgr::initEthernet(bool useDhcp, IPAddress staticIp,
...
void NetworkMgr::startFailoverTask() {
  if (failoverTaskHandle == nullptr) {
    // OPTIMIZATION: Moved to Core 0 to balance load (network monitoring
    // background task)
    xTaskCreatePinnedToCore(failoverTask, "NET_FAILOVER_TASK",
                            4096,  // Stack size
                            this,
                            1,  // Priority
                            &failoverTaskHandle,
...
void NetworkMgr::failoverLoop() {
  ...
  while (true) {
    ...
    if (now - lastReconnectCheck >= reconnectInterval) {
      ...
      if (wifiManager && wifiManager->hasStoredConfig() &&
          !wifiManager->isInitialized()) {
        wifiManager->tryReconnect();
      }

      // Try to reconnect Ethernet if it has stored config but is not
      // initialized
      if (ethernetManager && ethernetManager->hasStoredConfig() &&
          !ethernetManager->isInitialized()) {
    ...
    if (now - lastCheck >= failoverCheckInterval) {
      ...
      if (activeMode == "NONE") {
        ...
        if (!primaryAvailable) {
          Serial.printf(
              "Primary network (%s) lost. Attempting to switch to secondary.\n",
              primaryMode.c_str());
          String secondaryMode = (primaryMode == "ETH") ? "WIFI" : "ETH";
        ...
        if (!secondaryAvailable) {
          Serial.printf(
              "Secondary network (%s) lost. Attempting to switch to primary.\n",
              activeMode.c_str());
          if (primaryAvailable) {
            switchMode(primaryMode);
          }
...
uint32_t NetworkMgr::getReconnectInterval() const { return reconnectInterval; }
...
void NetworkMgr::updateWiFiSignalStrength() {
  ...
  unsigned long now = millis();
  if (now - wifiSignalMetrics.lastChecked >=
      wifiSignalMetrics.rssiCheckInterval) {
    wifiSignalMetrics.lastChecked = now;
    wifiSignalMetrics.rssi = getWiFiRSSI();
    wifiSignalMetrics.signalQuality =
        calculateSignalQuality(wifiSignalMetrics.rssi);
...
NetworkMgr::PooledClientConnection* NetworkMgr::getPooledClient(
  ...
  if (clientPool.size() < maxPoolSize) {
    ...
    LOG_NET_INFO(
        "[NetworkMgr] WARNING: Client pool full (%ld/%d), cannot add: %s\n",
...
int NetworkMgr::getPooledClientCount() const { return clientPool.size(); }
