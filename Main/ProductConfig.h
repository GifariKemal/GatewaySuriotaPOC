...
// Build number: Derived from version (1.3.2 = 132)
// Used for OTA version comparison
#define FIRMWARE_BUILD_NUMBER 132
...
class ProductInfo {
 public:
   ...
   */
  static const char* generateSerialNumber(const uint8_t* mac,
                                          const char* dateCode, char* buffer) {
    ...
    snprintf(buffer, 32, "%s-%s%s-%s-%s", SERIAL_PREFIX, SERIAL_MODEL_CODE,
             SERIAL_VARIANT_CODE, date, uid);
   ...
   */
  static uint32_t getBuildNumber() { return FIRMWARE_BUILD_NUMBER; }

  /**
   * @brief Get full product model string
   */
  static const char* getProductModel() { return PRODUCT_FULL_MODEL; }

  /**
   * @brief Check if this is POE variant
   */
  static bool isPOE() { return PRODUCT_IS_POE == 1; }

  /**
   * @brief Get manufacturer name
   */
  static const char* getManufacturer() { return MANUFACTURER_NAME; }
};
