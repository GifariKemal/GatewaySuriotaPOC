 ...
 * - State machine (IDLE → CHECKING → DOWNLOADING → VALIDATING → APPLYING →
 * REBOOTING)
 ...
 *
 * v2.5.35: ESP_SSLClient linker fix - forward declare OTAHttps, add ArduinoJson
 * include
 */
...
// This prevents ESP_SSLClient from being included in every file that includes
// OTAManager.h OTAHttps.h is only included in OTAManager.cpp
#include <Arduino.h>
...
class OTAManager;
class OTAHttps;  // v2.5.35: Forward declare to avoid ESP_SSLClient linker
                 // issues
class BLEServer;
...
struct OTAConfiguration {
  ...
  OTAConfiguration()
      : enabled(true),
        ...
        requireHttps(true),
        bleOtaEnabled(true),
        bleRequireButton(true),
        bleTimeoutMs(OTA_BLE_TIMEOUT_MS) {}
};

/**
 * @brief OTA Status for external queries
 * @version 2.5.37: Added bytesPerSecond, etaSeconds, networkMode, retryCount
 * for interactive UI
 */
struct OTAStatus {
  OTAState state;
...
class OTAManager {
 private:
  ...
  volatile bool
      checkTaskRunning;  // v2.5.1 FIX: Track if async check is running
  ...
  static void progressCallbackAdapter(uint8_t progress, size_t current,
                                      size_t total);
