...
inline void create(JsonDocument& doc, UnifiedErrorCode code,
  ...
  if (suggestion && strlen(suggestion) > 0 &&
      strcmp(suggestion, "Check logs for details") != 0) {
 ...
 */
inline void create(JsonDocument& doc, UnifiedErrorCode code,
                   const String& customMessage, const String& type = "") {
  create(doc, code, customMessage.isEmpty() ? nullptr : customMessage.c_str(),
 ...
 */
inline void createGeneric(JsonDocument& doc, const String& message,
...
inline void deviceNotFound(JsonDocument& doc, const String& deviceId = "") {
  create(doc, ERR_CFG_FILE_NOT_FOUND,
         deviceId.isEmpty() ? "Device not found"
                            : ("Device not found: " + deviceId).c_str(),
         "device");
}
...
inline void registerNotFound(JsonDocument& doc, const String& registerId = "") {
  create(doc, ERR_CFG_FILE_NOT_FOUND,
         registerId.isEmpty() ? "Register not found"
                              : ("Register not found: " + registerId).c_str(),
         "register");
}
...
inline void configSaveFailed(JsonDocument& doc, const String& details = "") {
  create(doc, ERR_CFG_SAVE_FAILED,
         details.isEmpty() ? nullptr : details.c_str(), "config");
}
...
inline void systemError(JsonDocument& doc, const String& message = "") {
  create(doc, ERR_SYS_UNKNOWN, message.isEmpty() ? nullptr : message.c_str(),
         "system");
}
...
inline void mqttConnectionFailed(JsonDocument& doc, const String& broker = "") {
  create(
      doc, ERR_MQTT_CONNECTION_FAILED,
