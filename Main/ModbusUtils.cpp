...
double ModbusUtils::processRegisterValue(const JsonObject &reg, uint16_t rawValue)
{
...
double ModbusUtils::processMultiRegisterValue(
    ...
    const char *endianness_variant)
{
  ...
  {
    uint32_t combined;
    if (strcmp(endianness_variant, "BE") == 0)
    { // Big Endian (ABCD)
      combined = ((uint32_t)values[0] << 16) | values[1];
    }
    else if (strcmp(endianness_variant, "LE") == 0)
    ...
    else if (strcmp(endianness_variant, "BE_BS") == 0)
    ...
    else if (strcmp(endianness_variant, "LE_BS") == 0)
  ...
  else if (count == 4)
  {
    ...
    { // Little Endian with Word Swap (W3, W2, W1, W0)
      combined = ((uint64_t)values[3] << 48) |
                 ((uint64_t)values[2] << 32) |
                 ((uint64_t)values[1] << 16) |
                 (uint64_t)values[0];
    }
    else
    { // Default to Big Endian
