...
enum UnifiedErrorCode {
  ...
  ERR_NET_NO_NETWORK_AVAILABLE =
      7,  // No network available (WiFi+Ethernet both down)
...
inline ErrorDomain getErrorDomain(UnifiedErrorCode code) {
  if (code < 100) return DOMAIN_NETWORK;
  if (code < 200) return DOMAIN_MQTT;
  if (code < 300) return DOMAIN_BLE;
  if (code < 400) return DOMAIN_MODBUS;
  if (code < 500) return DOMAIN_MEMORY;
  if (code < 600) return DOMAIN_CONFIG;
...
inline ErrorSeverity getDefaultSeverity(UnifiedErrorCode code) {
  ...
  if (code == ERR_MEM_UTILIZATION_CRITICAL ||
      code == ERR_MEM_SAFETY_MARGIN_BREACHED ||
      code == ERR_NET_NO_NETWORK_AVAILABLE ||
      code == ERR_SYS_WATCHDOG_TRIGGERED || code == ERR_SYS_THERMAL_CRITICAL) {
