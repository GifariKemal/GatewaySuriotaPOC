...
private:
  ...
  // These were previously static variables in publishQueueData() - moved to instance for thread safety
  struct PublishState
  {
  ...
  // v2.3.8 PHASE 1: Helper methods to eliminate code duplication (DRY principle)
  ...
  bool serializeAndValidatePayload(
      JsonDocument &doc,
      String &payload,
  ...
  void analyzeDeviceConfigurations(JsonArray &devices, uint32_t &maxRefreshRate, uint32_t &totalRegisters, bool &hasSlowRTU);
  uint32_t determineTimeoutStrategy(uint32_t totalRegisters, uint32_t maxRefreshRate, bool hasSlowRTU);
