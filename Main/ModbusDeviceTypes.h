...
struct ModbusDeviceFailureState
{
    ...
    explicit ModbusDeviceFailureState(const char *id)
        : deviceId(id) {}
};
...
struct ModbusDeviceHealthMetrics
{
    ...
    explicit ModbusDeviceHealthMetrics(const char *id)
        : deviceId(id) {}
...
namespace ModbusDeviceConfig
{
    ...
    constexpr unsigned long TCP_BASE_BACKOFF_MS = 2000;  // TCP: Slower network recovery
    constexpr unsigned long MAX_BACKOFF_MULTIPLIER = 32; // Max 32x base delay

    // Auto-recovery
    constexpr unsigned long AUTO_RECOVERY_CHECK_INTERVAL_MS = 60000; // Check every 1 minute
    constexpr unsigned long AUTO_RECOVERY_DELAY_MS = 300000;         // 5 minutes before retry
