...
OTAValidator* OTAValidator::getInstance() {
  if (!instance) {
    ...
    void* ptr = heap_caps_malloc(sizeof(OTAValidator),
...
bool OTAValidator::begin() {
  ...
  if (!loadPublicKeyFile(OTA_PUBLIC_KEY_FILE)) {
    ...
    if (!loadPublicKey(DEFAULT_PUBLIC_KEY_PEM)) {
      LOG_OTA_WARN(
          "No valid public key loaded - signature verification disabled\n");
    }
  }
...
int OTAValidator::loadPublicKeyFromPEM(const char* pemKey, size_t keyLen) {
  ...
  int ret =
      mbedtls_pk_parse_public_key(&pkContext, (const unsigned char*)pemKey,
...
int OTAValidator::loadPublicKeyFromFile(const char* filePath) {
  ...
  char* buffer = (char*)heap_caps_malloc(fileSize + 1,
                                         MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (buffer) {
    usedPsram = true;
  } else {
...
bool OTAValidator::verifySignatureHash(const uint8_t* hash,
  ...
  if (sigLen == OTA_SIGNATURE_SIZE) {
    ...
    ret = mbedtls_pk_verify(&pkContext, MBEDTLS_MD_SHA256, hash, OTA_HASH_SIZE,
                            derSig, derLen);
  } else {
...
void OTAValidator::computeHash(const uint8_t* data, size_t len,
                               uint8_t* hashOut) {
...
bool OTAValidator::validateHeader(const OTAFirmwareHeader* header,
  ...
  // Check firmware size
  if (header->firmwareSize == 0 ||
      header->firmwareSize > OTA_MAX_FIRMWARE_SIZE) {
    result.valid = false;
  ...
  LOG_OTA_INFO("Header valid: v%d.%d.%d (build %u), size %u\n",
               ...
               header->buildNumber, header->firmwareSize);
...
void OTAValidator::setCurrentVersion(uint8_t major, uint8_t minor,
                                     uint8_t patch, uint32_t buildNumber) {
  xSemaphoreTake(validatorMutex, portMAX_DELAY);
  currentMajor = major;
  currentMinor = minor;
  currentPatch = patch;
...
bool OTAValidator::isVersionNewer(uint8_t major, uint8_t minor, uint8_t patch,
  ...
  int cmp = compareVersions(major, minor, patch, currentMajor, currentMinor,
                            currentPatch);
  return cmp >= 0;
}
...
bool OTAValidator::validateFirmware(const uint8_t* firmwareData,
                                    size_t totalSize,
                                    ValidationResult& result) {
