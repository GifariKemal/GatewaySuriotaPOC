...
struct ValidationResult {
    ...
    ValidationResult() : valid(false), error(OTAError::NONE),
                        majorVersion(0), minorVersion(0), patchVersion(0),
                        buildNumber(0), firmwareSize(0) {}
};
...
public:
     ...
     * @return true if signature is valid
     */
    bool verifySignatureHash(const uint8_t* hash,
                            const uint8_t* signature, size_t sigLen);
     ...
     */
    bool validateHeader(const OTAFirmwareHeader* header, ValidationResult& result);
     ...
     */
    bool validateStreaming(const char* expectedHashHex,
                          const uint8_t* signature, size_t sigLen,
                          ValidationResult& result);
