...
void ModbusRtuService::storeRegisterValue(const String &deviceId, const JsonObject &reg, double value, const String &deviceName)
{
...
ModbusRtuService::DeviceFailureState *ModbusRtuService::getDeviceFailureState(const String &deviceId)
{
...
ModbusRtuService::DeviceReadTimeout *ModbusRtuService::getDeviceTimeout(const String &deviceId)
{
...
bool ModbusRtuService::configureDeviceBaudRate(const String &deviceId, uint16_t baudRate)
{
  ...
  Serial.printf("[RTU] Configured device %s baud rate to: %d\n", deviceId.c_str(), baudRate);
  return true;
}

uint16_t ModbusRtuService::getDeviceBaudRate(const String &deviceId)
{
  DeviceFailureState *state = getDeviceFailureState(deviceId);
  if (!state)
  {
...
void ModbusRtuService::handleReadFailure(const String &deviceId)
{
  DeviceFailureState *state = getDeviceFailureState(deviceId);
  if (!state)
    return;
...
bool ModbusRtuService::shouldRetryDevice(const String &deviceId)
{
  DeviceFailureState *state = getDeviceFailureState(deviceId);
...
void ModbusRtuService::resetDeviceFailureState(const String &deviceId)
{
  DeviceFailureState *state = getDeviceFailureState(deviceId);
  if (!state)
    return;
...
void ModbusRtuService::handleReadTimeout(const String &deviceId)
{
  DeviceReadTimeout *timeout = getDeviceTimeout(deviceId);
  if (!timeout)
    return;
...
bool ModbusRtuService::isDeviceEnabled(const String &deviceId)
{
  DeviceFailureState *state = getDeviceFailureState(deviceId);
  if (!state)
...
void ModbusRtuService::enableDevice(const String &deviceId)
{
  DeviceFailureState *state = getDeviceFailureState(deviceId);
  if (!state)
    return;
...
void ModbusRtuService::disableDevice(const String &deviceId)
{
  DeviceFailureState *state = getDeviceFailureState(deviceId);
  if (!state)
    return;
...
ModbusRtuService::DeviceTimer *ModbusRtuService::getDeviceTimer(const String &deviceId)
{
...
bool ModbusRtuService::shouldPollDevice(const String &deviceId, uint32_t refreshRateMs)
{
...
void ModbusRtuService::updateDeviceLastRead(const String &deviceId)
{
  DeviceTimer *timer = getDeviceTimer(deviceId);
  if (timer)
  {
