...
constexpr uint32_t CRUD_TASK_STACK_SIZE =
    32768;  // 32KB stack for CREATE operations with 50+ registers
}
...
enum class CommandPriority : uint8_t {
  ...
  PRIORITY_HIGH =
      0  // Time-sensitive operations (emergency stop, config update)
};
...
struct Command {
  ...
  String payloadJson;   // BUG #32 FIX: Store as serialized JSON String to avoid
                        // .set() corruption
  ...
  Command()
      : id(0),
        priority(CommandPriority::PRIORITY_NORMAL),
        enqueueTime(0),
        manager(nullptr) {}
...
class CRUDHandler {
 private:
  ...
  std::map<String, CommandHandler>
      controlHandlers;  // NEW: Device control operations
  std::map<String, CommandHandler>
  ...
  std::priority_queue<Command, std::vector<Command>, std::greater<Command>>
      commandQueue;
  ...
  void enqueueCommand(BLEManager* manager, const JsonDocument& command,
  ...
  void executeBatchSequential(BLEManager* manager, const String& batchId,
                              JsonArrayConst commands);
  void executeBatchAtomic(BLEManager* manager, const String& batchId,
                          JsonArrayConst commands);
  void logBatchStats();
  void updateQueueDepth();

 public:
  CRUDHandler(ConfigManager* config, ServerConfig* serverCfg,
  ...
  void setHttpManager(HttpManager* http) { httpManager = http; }

  // NEW: Set Modbus service references for device control
  void setModbusRtuService(ModbusRtuService* rtu) { modbusRtuService = rtu; }
  void setModbusTcpService(ModbusTcpService* tcp) { modbusTcpService = tcp; }
