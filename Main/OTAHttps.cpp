 ...
 *          When resume gets HTTP 200 instead of 206, must restart download from
 * byte 0 Reset hash computation and OTA partition on restart v2.5.15: Resume
 * download support (HTTP Range header) Retry count with automatic retry on
 * failure Visual progress bar in development mode v2.5.12: Fixed private repo
 * firmware download (HTTP 404) Use GitHub API releases/assets endpoint for
 * binary downloads Correct Accept header for asset downloads vs contents API
...
OTAHttps* OTAHttps::getInstance() {
  if (!instance) {
    void* ptr =
        heap_caps_malloc(sizeof(OTAHttps), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (ptr) {
...
bool OTAHttps::initSecureClient() {
  ...
  if (usingWiFi) {
    ...
    void* ptr = heap_caps_malloc(sizeof(ESP_SSLClient),
    ...
    wifiSecure->setBufferSizes(
        ESP_SSLCLIENT_BUFFER_SIZE,
        1024);  // RX 16KB (PSRAM), TX 1KB - larger buffer = faster download
    ...
    LOG_OTA_DEBUG(
        "ESP_SSLClient for WiFi configured, RX buffer: %d bytes (PSRAM)\n",
        ESP_SSLCLIENT_BUFFER_SIZE);
    ...
    ethSecure->setBufferSizes(ESP_SSLCLIENT_BUFFER_SIZE,
                              1024);       // RX 16KB (PSRAM), TX 1KB
    ...
    LOG_OTA_DEBUG(
        "ESP_SSLClient for Ethernet configured, RX buffer: %d bytes (PSRAM)\n",
        ESP_SSLCLIENT_BUFFER_SIZE);
...
bool OTAHttps::allocateBuffer() {
  ...
  // Try PSRAM first
  downloadBuffer = (uint8_t*)heap_caps_malloc(
...
String OTAHttps::buildRawUrl(const String& path) {
  // v2.5.11: For private repos, use GitHub API instead of
  // raw.githubusercontent.com raw.githubusercontent.com doesn't accept
  // authentication headers or token-in-URL GitHub API:
...
String OTAHttps::buildReleaseUrl(const String& tag, const String& filename) {
  // v2.5.11: For private repos, auth is handled via Authorization header in
  // performRequest() Format:
...
int OTAHttps::performRequest(const char* method, size_t rangeStart) {
  ...
  LOG_OTA_DEBUG("Connecting to %s:%d%s via %s\n", host.c_str(), port,
                path.c_str(), usingWiFi ? "WiFi" : "Ethernet");
...
bool OTAHttps::beginOTAPartition(size_t firmwareSize) {
  ...
  // We need to yield before and use OTA_SIZE_UNKNOWN to let esp-idf handle
  // erase internally
...
void OTAHttps::printProgressBar(uint8_t percent, size_t downloaded,
                                size_t total, uint32_t speed) {
...
bool OTAHttps::fetchManifestFromUrl(const String& url,
  ...
  // This is more robust than line-by-line reading which can fail with slow
  // connections
  String rawResponse = "";
  unsigned long timeout = millis() + readTimeoutMs;
...
bool OTAHttps::parseManifest(const String& json, FirmwareManifest& manifest) {
  ...
  // v2.5.11: Find JSON start - skip any remaining headers that weren't properly
  // skipped
  ...
  if (firmware["url"].is<String>() &&
      firmware["url"].as<String>().startsWith("http")) {
  ...
  LOG_OTA_INFO("Manifest parsed: v%s (build %u), size %u\n",
               manifest.version.c_str(), manifest.buildNumber,
...
bool OTAHttps::downloadFirmware(const FirmwareManifest& manifest,
  ...
  return downloadWithRetry(manifest.firmwareUrl, manifest.firmwareSize,
                           manifest.sha256Hash, manifest.signature, result);
}
...
bool OTAHttps::downloadWithRetry(const String& url, size_t expectedSize,
  ...
  while (currentRetryCount <= maxRetries) {
    ...
    // Attempt download
    bool success = downloadFirmwareFromUrl(url, expectedSize, expectedHash,
                                           signature, result);
    ...
    bool retryable =
        (lastError == OTAError::NETWORK_ERROR ||
         lastError == OTAError::SERVER_ERROR || lastError == OTAError::TIMEOUT);
...
bool OTAHttps::downloadFirmwareFromUrl(const String& url, size_t expectedSize,
  ...
  LOG_OTA_INFO("Expected size: %u, hash: %s\n", expectedSize,
               expectedHash.substring(0, 16).c_str());
  ...
  unsigned long lastDataTime =
      startTime;  // v2.5.6: Track last data received time
  ...
  // Keep reading as long as: (1) data available, OR (2) connected and not timed
  // out
  while (progress.bytesDownloaded < targetSize) {
    ...
    if (available > 0) {
      ...
      if (bytesRead > 0) {
        ...
        // Progress callback
        if (progressCallback &&
            (millis() - lastProgressTime >= OTA_PROGRESS_INTERVAL_MS)) {
          progressCallback(progress.percent, progress.bytesDownloaded,
                           targetSize);
          lastProgressTime = millis();
        }

        // v2.5.15: Visual progress bar (dev mode only, every 5%)
        printProgressBar(progress.percent, progress.bytesDownloaded, targetSize,
      ...
      // IMPORTANT: GitHub API does NOT support Range requests - always returns
      // HTTP 200 If we get 200 instead of 206, server is sending entire file
      // from start We must reset hash and progress to avoid checksum mismatch
      if (!sslClient->connected()) {
        if (progress.bytesDownloaded < targetSize) {
          LOG_OTA_WARN(
              "Connection lost at %lu/%lu bytes. Attempting resume...\n",
          ...
          if (setupHttpClient(serverUrl)) {
            ...
            if (httpCode == 206) {
              ...
              LOG_OTA_WARN(
                  "Server returned 200 (no Range support). Restarting download "
                  "from byte 0\n");
              ...
              if (newContentLength > 0 && newContentLength != targetSize) {
                LOG_OTA_WARN(
                    "Content-Length changed: %u -> %u, using new value\n",
  ...
  if (progress.bytesDownloaded != targetSize) {
    ...
    LOG_OTA_ERROR("Incomplete: %u / %u (%s)\n", progress.bytesDownloaded,
                  contentLength, lastErrorMessage.c_str());
    abortOTA();
    downloading = false;
    progress.inProgress = false;
    xSemaphoreGive(httpMutex);
  ...
  if (validator && (expectedHash.length() > 0 || signature.length() > 0)) {
    ...
    if (!validator->validateStreaming(expectedHash.c_str(), sigBytes, sigLen,
                                      result)) {
...
bool OTAHttps::checkForUpdate(FirmwareManifest& manifest) {
  ...
  if (validator) {
    ...
    if (parseVersion(manifest.version.c_str(), major, minor, patch)) {
      if (!validator->isVersionNewer(major, minor, patch,
