 ...
 * @date 2025-12-12
 *
 * v2.5.35: Include OTAHttps.h here instead of in OTAManager.h to avoid
 * ESP_SSLClient linker errors
 */
...
OTAManager* OTAManager::getInstance() {
  if (!instance) {
    void* ptr = heap_caps_malloc(sizeof(OTAManager),
...
bool OTAManager::loadConfiguration() {
  ...
  if (!server.isNull()) {
    config.checkIntervalHours =
        server["check_interval_hours"] | OTA_CHECK_INTERVAL_HOURS;
...
void OTAManager::setState(OTAState newState) {
  if (currentState != newState) {
    ...
    LOG_OTA_INFO("OTA State: %s -> %s\n", getOTAStateName(oldState),
                 getOTAStateName(newState));
...
bool OTAManager::startUpdateFromUrl(const String& url) {
  ...
  bool success =
      httpsTransport->downloadFirmwareFromUrl(url, 0, "", "", result);
...
bool OTAManager::performHttpsUpdate(const String& url) {
  ...
  updateMode = url.length() > 0 ? OTAUpdateMode::HTTPS_CUSTOM
                                : OTAUpdateMode::HTTPS_GITHUB;
  setState(OTAState::DOWNLOADING);
  clearError();
...
void OTAManager::progressCallbackAdapter(uint8_t prog, size_t current,
                                         size_t total) {
  if (callbackInstance) {
    ...
    if (callbackInstance->bleNotificationManager &&
      ...
      if (callbackInstance->httpsTransport) {
        DownloadProgress dlProgress =
            callbackInstance->httpsTransport->getProgress();
...
String OTAManager::handleMqttCommand(const String& action,
                                     const String& params) {
...
bool OTAManager::handleCrudCommand(const String& command,
...
void OTAManager::setAutoUpdate(bool enabled) { config.autoUpdate = enabled; }
