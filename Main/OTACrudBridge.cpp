 ...
 * @date 2025-12-12
 *
 * This file contains all OTA operations and is the ONLY file that includes
 * OTAManager.h. This prevents ESP_SSLClient linker errors caused by non-inline
 * functions in Helper.h.
 *
...
OTAManager* getInstance() { return OTAManager::getInstance(); }
...
bool checkUpdate(OTAManager* otaManager, JsonDocument& response) {
  if (!otaManager) {
    ...
    ErrorResponse::create(response, ERR_SYS_UNKNOWN,
                          "OTA Manager not initialized", "ota");
    return false;
  }
...
bool startUpdate(OTAManager* otaManager, const String& customUrl,
  ...
  if (!netMgr || !netMgr->isAvailable()) {
    ...
    ErrorResponse::create(
        response, ERR_NET_NO_NETWORK_AVAILABLE,
  ...
  if (networkMode == "WIFI") {
    ...
    if (quality < 30) {
      LOG_OTA_INFO(
          "[OTA] Warning: WiFi signal weak (%d%%). OTA may be slow or fail.",
          quality);
      response["warning"] =
          "WiFi signal weak. Consider using Ethernet for reliable OTA.";
    }
  }
  ...
  if (!customUrl.isEmpty()) {
    LOG_OTA_INFO("[OTA] Starting update from custom URL: %s",
                 customUrl.c_str());
    started = otaManager->startUpdateFromUrl(customUrl);
  } else {
    LOG_OTA_INFO("[OTA] Starting update from GitHub (network: %s)...",
                 networkMode.c_str());
  ...
  if (started) {
    response["message"] =
        "OTA update started. Monitor progress with ota_status command.";
...
void confirmUpdate(OTAManager* otaManager, JsonDocument& response) {
  ...
  // Valid states for apply: VALIDATING (just finished download) or IDLE with
  // pending update
  if (state == OTAState::VALIDATING ||
    ...
    response["error_message"] =
        "No firmware ready to apply. Run check_update and start_update first.";
...
void enableBleOta(OTAManager* otaManager, JsonDocument& response) {
  ...
  response["message"] =
      "BLE OTA mode enabled. Device is ready to receive firmware via BLE.";
...
void setGitHubRepo(OTAManager* otaManager, const String& owner,
  ...
  LOG_OTA_INFO("[OTA] GitHub repo updated: %s/%s (branch: %s)", owner.c_str(),
               repo.c_str(), branch.c_str());
}
...
void setGitHubToken(OTAManager* otaManager, const String& token,
  ...
  LOG_OTA_INFO("[OTA] GitHub token %s",
               token.isEmpty() ? "cleared" : "updated");
}
