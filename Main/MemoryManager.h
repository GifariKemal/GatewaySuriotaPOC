...
// Include PSRAM Validator for memory safety
...
/*
 * @brief Factory function to create an object in PSRAM and wrap it in a
 PsramUniquePtr.
 *
 ...
 * @return A PsramUniquePtr managing the newly created object, or nullptr on
 failure.
 */
template <typename T, typename... Args>
PsramUniquePtr<T> make_psram_unique(Args&&... args) {
  // Pre-allocation validation via PSRAMValidator
  PSRAMValidator* validator = PSRAMValidator::getInstance();
  if (!validator->canAllocate(sizeof(T), "make_psram_unique")) {
    return nullptr;  // Allocation check failed
  }

  // Safe allocation using validator
  void* mem = validator->allocatePSRAM(sizeof(T), "make_psram_unique");
  if (!mem) {
