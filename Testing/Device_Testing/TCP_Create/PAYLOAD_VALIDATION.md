...
**⚠️ UPDATE:** `refresh_rate_ms` removed from register - tidak lagi supported
(hanya di device level)
...
**Note:** `refresh_rate_ms` removed - tidak lagi supported di register level
(hanya device level)
...
3. **`refresh_rate_ms`:** ✅ REMOVED - tidak lagi supported di register (hanya
   device level)
...
**Validation Date:** 2025-11-14 **API Version:** v2.3.0 **Validator:** Kemal -
SURIOTA R&D Team
