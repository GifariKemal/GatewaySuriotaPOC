...
# --- Configuration ---
SLAVE_ADDRESS = 1
SERIAL_PORT = 'COM8'  # Change this to your virtual serial port (e.g., 'COM2', '/dev/ttyUSB1')
BAUD_RATE = 9600
...
def _bytes_to_uint16_registers(byte_data, word_order):
    ...
    for i in range(0, len(byte_data), 2):
        registers.append(int.from_bytes(byte_data[i:i+2], byteorder='big')) # Modbus registers are big-endian
    
    if word_order == "le": # Little-endian word order (R2 R1 for 32-bit, R4 R3 R2 R1 for 64-bit)
        registers.reverse()
...
class ModbusSlaveSimulator:
    def __init__(self, port, slave_address):
        ...
        print(f"Modbus RTU Slave Simulator initialized on {port} (Slave ID: {slave_address})")
        print(f"Baudrate: {BAUD_RATE}, Parity: {PARITY}, Data bits: {DATA_BITS}, Stop bits: {STOP_BITS}")
    ...
    def _setup_registers(self):
        ...
        for data_type_str, value in TEST_VALUES.items():
            if "bool" in data_type_str:
                ...
                print(f"Holding Registers {address_counter}-{address_counter + len(registers) - 1} ({data_type_str}): {value} -> Raw: {registers}")
            
            # Increment address counter by the number of registers used for this data type
            if "int16" in data_type_str or "uint16" in data_type_str or "bool" in data_type_str:
                address_counter += 1
            elif "int32" in data_type_str or "uint32" in data_type_str or "float32" in data_type_str:
                address_counter += 2
            elif "int64" in data_type_str or "uint64" in data_type_str or "float64" in data_type_str:
                address_counter += 4
        
        print("\n--- Initialized Modbus Registers ---")
        print("Holding Registers:", {k: hex(v) for k, v in self.holding_registers.items()})
        ...
        if function_code == 3:  # Read Holding Registers
            ...
            for i in range(quantity):
                ...
                if address in self.holding_registers:
                    ...
                    response_bytes.extend(value.to_bytes(2, byteorder='big')) # Modbus registers are big-endian
                else:
            ...
            if start_address in self.coils:
                ...
                response_bytes = [function_code, 1, 0x01 if value else 0x00] # Function code, byte count, coil status
                return bytes(response_bytes)
            else:
        ...
        while True:
            try:
                ...
                if request:
                    ...
                    if len(request) < 8: # Minimum request length (slave ID, func code, address, quantity, CRC)
                        continue
                    ...
                    if response_pdu:
                        ...
                        response_frame += crc.to_bytes(2, byteorder='little') # CRC is little-endian
